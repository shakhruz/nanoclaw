...
const POLICY = {
  ...
  web_deploy:          { auto: true,  risk: 'low'    }, // vercel --prod, idempotent (per voice 691 — Webmaster autonomous)
...
// ───────── web_deploy executor — vercel --prod for milagpt.cc ─────────
//
// Webmaster (или любой Mila в Octo/Clients/Partners) дёргает этот action,
// когда нужно запушить свежий /workspace/global/web-projects/milagpt-cc/
// на milagpt.cc. Auto-approve (per voice 691): Webmaster — это его работа.
//
// Params:
//   project: "milagpt-cc"  (default — единственный публичный домен сейчас)
//   reason:  свободный текст для ledger
executors.web_deploy = async function (req) {
  const project = req.params?.project || 'milagpt-cc';
  // Vercel projects live under ~/apps/<project>/ (host territory, not in repo)
  const projectDir = path.join(os.homedir(), 'apps', project);
  if (!fs.existsSync(projectDir)) {
    return { ok: false, err: `project dir not found: ${projectDir}` };
  }
  // Use vercel via npx to avoid global-install dependency. Run from project dir.
  // Cap timeout at 5 min — typical milagpt.cc build is 30-90s.
  const cmd = `cd ${JSON.stringify(projectDir)} && npx vercel deploy --prod --scope milagpt --yes 2>&1`;
  const r = execBash(cmd, { timeout: 300_000 });
  if (!r.ok) return { ok: false, err: r.err, stderr: r.stderr, stdout: r.out };

  // Extract production URL from vercel output (last "https://" line typically)
  const urls = (r.out.match(/https:\/\/[^\s]+/g) || []);
  const prodUrl = urls[urls.length - 1] || null;
  return {
    ok: true,
    result: {
      project,
      production_url: prodUrl,
      output_tail: r.out.split('\n').slice(-10).join('\n'),
    },
  };
};
